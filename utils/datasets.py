...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        # =====================================================================================================
        # self.flip_index = [0, 2, 1, 4, 3, 6, 5, 8, 7, 10, 9, 12, 11, 14, 13, 16, 15] # pose에 맞춰서 수정 필요
        # ==================== youngmin ======================================================================
            ...
            try:
                ...
                if os.path.isfile(lb_file):
                    ...
                    if len(l):
                        ...
                        if kpt_label:
                            assert l.shape[1] == 5+3*n_kpts, f'labels require {5+3*n_kpts} columns each'
                            ...
                            # edited by yym
                            # kpts = np.zeros((l.shape[0], 39)) # 39는 어디서 나온 숫자인가? 17개의 visibility parameter를 제외한 것임. n_kpt를 빼준 숫자.
                            kpts = np.zeros((l.shape[0], 5+2*n_kpts)) 
                            for i in range(len(l)):
                                kpt = np.delete(l[i,5:], np.arange(2, l.shape[1]-5, 3))  #remove the occlusion paramater from the GT
                                # yym: by the result of the right above line, the occlusion parameter of the label is removed and the label length becomes (2*n_kpts + 5).
                                kpts[i] = np.hstack((l[i, :5], kpt))
                            l = kpts
                            assert l.shape[1] == 5+2*n_kpts, 'labels require 5+2n_kpts columns each after removing occlusion paramater'
                        else:
                        ...
                        ne += 1  # label empty
                        l = np.zeros((0, 5+2*n_kpts), dtype=np.float32) if kpt_label else np.zeros((0, 5), dtype=np.float32)
    ...
    if n:
        ...
        if use_segments:  # warp segments
            ...
            if kpt_label:
                ...
                xy_kpts[:, :2] = targets[:,5:].reshape(n*n_kpts, 2)  
